    ...
    const rows = await query(
      `select p.id, p.workspace_id, p.user_id, p.title, p.brief, p.created_at, p.updated_at,
        (select row_to_json(r) from (
          select id, readiness, summary, disciplines, controls, blockers, implementation_prompt, created_at
...
export async function saveReview({ userId, projectId, review }) {
  ...
  if (pool) {
    ...
    const [saved] = await query(
      `insert into reviews (id, project_id, readiness, summary, disciplines, controls, blockers, implementation_prompt, created_at)
       values ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       returning id, project_id, readiness, summary, disciplines, controls, blockers, implementation_prompt, created_at`,
      [
        review.id,
        projectId,
        review.readiness,
