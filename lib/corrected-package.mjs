...
export function correctedPackageZip(project, review) {
  const files = [
    fileRecord(
      "readme.md",
      `# otherend corrected package\n\nthis is an otai paid-tier handoff package for "${project.title}".\n\nit does not secretly rewrite your whole app. it gives your builder or coding tool a corrected approach, safer plan, manual quality tester script, and automated testing instructions based on the review.\n`,
    ),
    fileRecord(
    ...
    fileRecord(
      "manual-quality-test-plan.md",
      `# manual quality test plan\n\nuse this when a real person is clicking through the app before launch.\n\n${(review.manualTests || []).map((item, index) => `${index + 1}. ${item}`).join("\n")}\n`,
    ),
    fileRecord(
      "automated-test-plan.md",
      `# automated test plan\n\nuse this to create repeatable tests in playwright, api tests, database tests, and permission boundary tests.\n\n${(review.automatedTests || []).map((item) => `- ${item}`).join("\n")}\n\n## suggested starter command\n\n\`\`\`bash\nnpm run test\n\`\`\`\n\n## suggested browser test shape\n\n\`\`\`ts\ntest("main user flow works", async ({ page }) => {\n  await page.goto(process.env.APP_URL || "http://localhost:3000");\n  // sign in, create the main record, confirm success, then test a failure path.\n});\n\`\`\`\n`,
    ),
    fileRecord(
      "launch-checklist.md",
